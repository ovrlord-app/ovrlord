...
### Report bugs using GitHub's [issues](https://github.com/ovrlord-app/ovrlord/issues)
...
the [tags on this repository](https://github.com/ovrlord-app/ovrlord/tags).
...
- Ovrlords's priorities in architectural design are self-hosted and always open-source.
    - Avoid vendor lock-in, use open standards such as open telemetry.
    - Keep the infrastructure requirements minimal, e.g. only requires Postgres, no cloud provider specific features.
...
- The Javascript framework is [React](https://react.dev/) transpiled
  with [Typescript](https://www.typescriptlang.org/)
- No CSS framework
- All UI code is linted with `npm run lint` and formatted with `npm run format:fix` on every PR before merging.
- All UI unit tests must pass on every PR before merging.

### End-to-End tests

End-to-End testing utilizes [Playwright](https://playwright.dev/) and aid in validating that Ovrlord's primary
...
See the list of [contributors](https://github.com/ovrlord-app/ovrlord/contributors) who
participated in
this project.
